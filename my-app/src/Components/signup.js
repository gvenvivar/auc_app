...
const SignUp = React.forwardRef(({signUpPassMatch, passwordIsMatch, signUp}, ref) => {
	const {emailRef, passwordRef, passwordRepeatRef, errorRef} = ref;

	function checkPasswords(){
		const pass = passwordRef.current;
		const passR = passwordRepeatRef.current;
		const error = errorRef.current;

		if(pass.value === passR.value && pass.value !==''){
			error.style.color = 'green';
			signUpPassMatch(true);
		}
		else{
			error.style.color = 'red';
			signUpPassMatch(false);
		}
	}
	...
	return (
      ...
      <label>
        <span>Password</span>
        <input type='password' name='password' placeholder='Password' id='psw' onChange={checkPasswords} ref={passwordRef} required/>
      </label>
      <label className='repeatPass'>
        <span>Re-enter password</span>
        <input type='password' name='passwordR' placeholder='Password' id='pswR' onChange={checkPasswords} ref={passwordRepeatRef} required/>
      </label>
      <div className='error' ref={errorRef}>{passwordIsMatch}</div>
      <div className='btn-wrap'>
        <button className='btn' id='login' onClick={signUp}>Sign Up<img className='loading' src={loading} alt='loading'/></button>
      </div>
    </form>
	)
})
